 ...
 * workload: fp_w_connman_wifi
 * scenario: [P] webOS-faithful: 네트워크 매니저(connman/wpa 대응) — NETLINK_ROUTE 구독 + 버스
 * features: netlink,unix_listen,epoll
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_netlink_l01"); phase_gap(gap_ms);
      if (fd < 0) die("netlink socket");
      PHASE("f_netlink_l02"); phase_gap(gap_ms);
      struct sockaddr_nl a; memset(&a, 0, sizeof(a)); a.nl_family = AF_NETLINK;
      PHASE("f_netlink_l03"); phase_gap(gap_ms);
      a.nl_groups = RTMGRP_LINK;
      PHASE("f_netlink_l04"); phase_gap(gap_ms);
      if (bind(fd, (struct sockaddr *)&a, sizeof(a)) < 0) die("netlink bind");
      PHASE("f_netlink_l05"); phase_gap(gap_ms);
      KEEPFD(fd); }
    PHASE("f_netlink"); phase_gap(gap_ms);
