 ...
 * workload: fp_c_broker
 * scenario: IPC 브로커: 내부 채널 + UNIX listen + 이벤트 루프
 * features: socketpair,unix_listen,epoll
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_socketpair_l01"); phase_gap(gap_ms);
      if (write(sv[0], "x", 1) != 1) die("sp write");
      KEEPFD(sv[0]); KEEPFD(sv[1]); }
    PHASE("f_socketpair"); phase_gap(gap_ms);
