 ...
 * workload: fp_c_ads
 * scenario: 광고/비콘 모듈: UDP + 주기 타이머 + 로그
 * features: udp,itimer,logfile
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_itimer_l01"); phase_gap(gap_ms);
      if (sigaction(SIGALRM, &sa, NULL) < 0) die("sigaction ALRM");
      PHASE("f_itimer_l02"); phase_gap(gap_ms);
      struct itimerval iv = { { 0, 200000 }, { 0, 200000 } };
      PHASE("f_itimer_l03"); phase_gap(gap_ms);
      if (setitimer(ITIMER_REAL, &iv, NULL) < 0) die("setitimer");
      }
    PHASE("f_itimer"); phase_gap(gap_ms);
