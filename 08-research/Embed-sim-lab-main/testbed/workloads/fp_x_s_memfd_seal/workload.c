 ...
 * workload: fp_x_s_memfd_seal
 * scenario: v2 extension probe: 봉인된 memfd (F_SEAL_WRITE — 불변 공유버퍼 프로토콜)
 * features: memfd_seal
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_memfd_seal_l01"); phase_gap(gap_ms);
      if (m < 0) die("memfd_seal create");
      PHASE("f_memfd_seal_l02"); phase_gap(gap_ms);
      if (ftruncate(m, 1 << 20) < 0) die("memfd_seal trunc");
      PHASE("f_memfd_seal_l03"); phase_gap(gap_ms);
      if (fcntl(m, 1033 /* F_ADD_SEALS */, 1|2|8 /* SEAL|SHRINK|GROW|WRITE */) < 0) die("memfd_seal seal");
      PHASE("f_memfd_seal_l04"); phase_gap(gap_ms);
      void *mp = mmap(NULL, 1 << 20, PROT_READ, MAP_SHARED, m, 0);
      PHASE("f_memfd_seal_l05"); phase_gap(gap_ms);
      if (mp == MAP_FAILED) die("memfd_seal mmap");
      PHASE("f_memfd_seal_l06"); phase_gap(gap_ms);
      KEEPFD(m); KEEP(mp); }
    PHASE("f_memfd_seal"); phase_gap(gap_ms);
