 ...
 * workload: fp_c_settings
 * scenario: 설정 서비스: 메시지 큐 + 레거시 세마포어 + 로그
 * features: mq,sem_sysv,logfile
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_mq_l01"); phase_gap(gap_ms);
      mq_unlink(g_path_mq);
      PHASE("f_mq_l02"); phase_gap(gap_ms);
      struct mq_attr at = { .mq_flags = 0, .mq_maxmsg = 4, .mq_msgsize = 64 };
      PHASE("f_mq_l03"); phase_gap(gap_ms);
      mqd_t q = mq_open(g_path_mq, O_CREAT | O_RDWR | O_NONBLOCK, 0600, &at);
      PHASE("f_mq_l04"); phase_gap(gap_ms);
      if (q == (mqd_t)-1) die("mq_open");
      PHASE("f_mq_l05"); phase_gap(gap_ms);
      if (mq_send(q, "cmd", 4, 0) < 0) die("mq_send");
      KEEPFD((int)q); }
    PHASE("f_mq"); phase_gap(gap_ms);
    { int id = semget(IPC_PRIVATE, 1, IPC_CREAT | 0600); if (id < 0) die("semget");
    PHASE("f_sem_sysv_l01"); phase_gap(gap_ms);
      union semun { int val; } arg; arg.val = 1;
      PHASE("f_sem_sysv_l02"); phase_gap(gap_ms);
      if (semctl(id, 0, SETVAL, arg) < 0) die("semctl SETVAL");
      g_semid = id; }
    PHASE("f_sem_sysv"); phase_gap(gap_ms);
