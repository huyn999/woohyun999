 ...
 * workload: fp_p24_shm_sysv__timerfd
 * scenario: pair probe: SysV 공유메모리, attach 후 IPC_RMID (오디오/AV 파이프라인 레거시) + 주기 timerfd (프레임/폴링 타이머)
 * features: shm_sysv,timerfd
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_shm_sysv_l01"); phase_gap(gap_ms);
      if (id < 0) die("shmget");
      PHASE("f_shm_sysv_l02"); phase_gap(gap_ms);
      char *p = shmat(id, NULL, 0); if (p == (void *)-1) die("shmat");
      PHASE("f_shm_sysv_l03"); phase_gap(gap_ms);
      shmctl(id, IPC_RMID, NULL);   /* attach 유지 중 RMID — 마지막 detach 때 소멸 */
      for (size_t i = 0; i < (16ull << 20); i += 4096) p[i] = 4;
      PHASE("f_shm_sysv_l04"); phase_gap(gap_ms);
      KEEP(p); }
    PHASE("f_shm_sysv"); phase_gap(gap_ms);
