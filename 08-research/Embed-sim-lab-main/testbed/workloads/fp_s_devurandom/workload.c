 ...
 * workload: fp_s_devurandom
 * scenario: single-feature probe: /dev/urandom fd (난수 소스 상시 오픈)
 * features: devurandom
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_devurandom_l01"); phase_gap(gap_ms);
      char b[16]; if (read(fd, b, sizeof(b)) < 0) die("urandom read"); KEEPFD(fd); }
    PHASE("f_devurandom"); phase_gap(gap_ms);
