 ...
 * workload: fp_c_appstore
 * scenario: 앱스토어 다운로더: 서버 연결 + 다운로드 임시파일 + 로그
 * features: tcp_established,tmpunlink,logfile
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_tmpunlink_l01"); phase_gap(gap_ms);
      if (fd < 0) die("mkstemp");
      char b[4096]; memset(b, 7, sizeof(b));
      PHASE("f_tmpunlink_l02"); phase_gap(gap_ms);
      if (write(fd, b, sizeof(b)) != sizeof(b)) die("ghost write");
      PHASE("f_tmpunlink_l03"); phase_gap(gap_ms);
      if (unlink(tmpl) < 0) die("ghost unlink");
      KEEPFD(fd); }
    PHASE("f_tmpunlink"); phase_gap(gap_ms);
