 ...
 * workload: fp_x_s_lease_rd
 * scenario: v2 extension probe: 파일 read lease (F_SETLEASE — 캐시 무결성 프로토콜)
 * features: lease_rd
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_lease_rd_l01"); phase_gap(gap_ms);
      int lf = open(g_path_map, O_CREAT | O_RDONLY, 0600); if (lf < 0) die("lease open");
      PHASE("f_lease_rd_l02"); phase_gap(gap_ms);
      int lr = fcntl(lf, F_SETLEASE, F_RDLCK);
      PHASE("f_lease_rd_l03"); phase_gap(gap_ms);
      KEEPFD(lf);
      PHASE("f_lease_rd_l04"); phase_gap(gap_ms);
      printf("PHASE_NOTE lease ok=%d errno=%d\n", lr == 0, lr == 0 ? 0 : errno);
      PHASE("f_lease_rd_l05"); phase_gap(gap_ms);
      fflush(stdout); }
    PHASE("f_lease_rd"); phase_gap(gap_ms);
