 ...
 * workload: fp_x_s_epoll_excl
 * scenario: v2 extension probe: EPOLLEXCLUSIVE 등록 (thundering-herd 회피 관용구)
 * features: epoll_excl
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_epoll_excl_l01"); phase_gap(gap_ms);
      int ev = eventfd(0, 0); if (ev < 0) die("epx eventfd");
      PHASE("f_epoll_excl_l02"); phase_gap(gap_ms);
      struct epoll_event e; memset(&e, 0, sizeof(e));
      PHASE("f_epoll_excl_l03"); phase_gap(gap_ms);
      e.events = EPOLLIN | (1U << 28) /* EPOLLEXCLUSIVE */; e.data.fd = ev;
      PHASE("f_epoll_excl_l04"); phase_gap(gap_ms);
      if (epoll_ctl(ep, EPOLL_CTL_ADD, ev, &e) < 0) die("epx ctl");
      PHASE("f_epoll_excl_l05"); phase_gap(gap_ms);
      KEEPFD(ep); KEEPFD(ev); }
    PHASE("f_epoll_excl"); phase_gap(gap_ms);
