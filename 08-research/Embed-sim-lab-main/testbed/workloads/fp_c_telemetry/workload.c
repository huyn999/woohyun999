 ...
 * workload: fp_c_telemetry
 * scenario: 텔레메트리: UDP 송신 + 로그 + RT 타이머
 * features: udp,logfile,posix_timer
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_udp_l01"); phase_gap(gap_ms);
      struct sockaddr_in a; memset(&a, 0, sizeof(a)); a.sin_family = AF_INET;
      PHASE("f_udp_l02"); phase_gap(gap_ms);
      a.sin_addr.s_addr = htonl(INADDR_LOOPBACK); a.sin_port = 0;
      PHASE("f_udp_l03"); phase_gap(gap_ms);
      if (bind(fd, (struct sockaddr *)&a, sizeof(a)) < 0) die("udp bind");
      KEEPFD(fd); }
    PHASE("f_udp"); phase_gap(gap_ms);
