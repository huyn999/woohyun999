 ...
 * workload: fp_x_s_raw_icmp
 * scenario: v2 extension probe: raw ICMP 소켓 (네트워크 진단 데몬)
 * features: raw_icmp
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_raw_icmp_l01"); phase_gap(gap_ms);
      if (r >= 0) KEEPFD(r);
      PHASE("f_raw_icmp_l02"); phase_gap(gap_ms);
      printf("PHASE_NOTE raw_icmp ok=%d errno=%d\n", r >= 0, r >= 0 ? 0 : errno);
      PHASE("f_raw_icmp_l03"); phase_gap(gap_ms);
      fflush(stdout); }
    PHASE("f_raw_icmp"); phase_gap(gap_ms);
