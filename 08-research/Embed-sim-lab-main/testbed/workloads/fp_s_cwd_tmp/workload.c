 ...
 * workload: fp_s_cwd_tmp
 * scenario: single-feature probe: 작업 디렉터리 이동 (앱 데이터 디렉터리)
 * features: cwd_tmp
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_cwd_tmp_l01"); phase_gap(gap_ms);
      mkdir(g_path_cwd, 0755);
      PHASE("f_cwd_tmp_l02"); phase_gap(gap_ms);
      if (chdir(g_path_cwd) < 0) die("chdir");
      }
    PHASE("f_cwd_tmp"); phase_gap(gap_ms);
