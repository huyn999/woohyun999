 ...
 * workload: fp_s_fifo
 * scenario: single-feature probe: named pipe (레거시 IPC)
 * features: fifo
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_fifo_l01"); phase_gap(gap_ms);
      unlink(g_path_fifo);
      PHASE("f_fifo_l02"); phase_gap(gap_ms);
      if (mkfifo(g_path_fifo, 0600) < 0) die("mkfifo");
      PHASE("f_fifo_l03"); phase_gap(gap_ms);
      int fd = open(g_path_fifo, O_RDWR); if (fd < 0) die("fifo open"); KEEPFD(fd); }
    PHASE("f_fifo"); phase_gap(gap_ms);
