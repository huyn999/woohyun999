 ...
 * workload: fp_s_abstract_unix
 * scenario: single-feature probe: abstract namespace UNIX listen (경로 없는 버스)
 * features: abstract_unix
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_abstract_unix_l01"); phase_gap(gap_ms);
      struct sockaddr_un a; memset(&a, 0, sizeof(a)); a.sun_family = AF_UNIX;
      PHASE("f_abstract_unix_l02"); phase_gap(gap_ms);
      int n = snprintf(a.sun_path + 1, sizeof(a.sun_path) - 1, "criuprobe_abs_p%d", g_port);
      PHASE("f_abstract_unix_l03"); phase_gap(gap_ms);
      if (bind(fd, (struct sockaddr *)&a, sizeof(sa_family_t) + 1 + n) < 0) die("aus bind");
      PHASE("f_abstract_unix_l04"); phase_gap(gap_ms);
      if (listen(fd, 4) < 0) die("aus listen");
      KEEPFD(fd); }
    PHASE("f_abstract_unix"); phase_gap(gap_ms);
