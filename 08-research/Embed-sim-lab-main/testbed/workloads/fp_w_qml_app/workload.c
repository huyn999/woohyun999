 ...
 * workload: fp_w_qml_app
 * scenario: [P] webOS-faithful: QML 시스템 UI 앱 — 씬 리소스 매핑 + 렌더 스레드 + 버스
 * features: large_heap,thread4,mmap_priv,unix_conn
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_mmap_priv_l01"); phase_gap(gap_ms);
      int fd = open(g_path_map, O_CREAT | O_RDWR, 0644); if (fd < 0) die("map open");
      PHASE("f_mmap_priv_l02"); phase_gap(gap_ms);
      size_t n = 8ull << 20; if (ftruncate(fd, n) < 0) die("map ftruncate");
      PHASE("f_mmap_priv_l03"); phase_gap(gap_ms);
      char *p = mmap(NULL, n, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
      PHASE("f_mmap_priv_l04"); phase_gap(gap_ms);
      if (p == MAP_FAILED) die("map mmap");
      PHASE("f_mmap_priv_l05"); phase_gap(gap_ms);
      for (size_t i = 0; i < n; i += 4096) p[i] = 5;
      PHASE("f_mmap_priv_l06"); phase_gap(gap_ms);
      KEEP(p); close(fd); }
    PHASE("f_mmap_priv"); phase_gap(gap_ms);
