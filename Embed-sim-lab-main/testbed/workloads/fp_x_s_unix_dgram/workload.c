 ...
 * workload: fp_x_s_unix_dgram
 * scenario: v2 extension probe: UNIX 데이터그램 쌍 + 미수신 패킷
 * features: unix_dgram
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_unix_dgram_l01"); phase_gap(gap_ms);
      if (send(sp[0], "evt", 3, 0) != 3) die("dgram send");
      PHASE("f_unix_dgram_l02"); phase_gap(gap_ms);
      KEEPFD(sp[0]); KEEPFD(sp[1]); }
    PHASE("f_unix_dgram"); phase_gap(gap_ms);
