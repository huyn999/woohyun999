 ...
 * workload: fp_x_s_sig_pending
 * scenario: v2 extension probe: 블록된 RT 시그널 3개 pending (미처리 이벤트 큐)
 * features: sig_pending
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_sig_pending_l01"); phase_gap(gap_ms);
      if (sigprocmask(SIG_BLOCK, &bs, NULL) < 0) die("sig block");
      PHASE("f_sig_pending_l02"); phase_gap(gap_ms);
      union sigval v; v.sival_int = 7;
      PHASE("f_sig_pending_l03"); phase_gap(gap_ms);
      if (sigqueue(getpid(), SIGRTMIN, v) < 0) die("sigqueue1");
      PHASE("f_sig_pending_l04"); phase_gap(gap_ms);
      if (sigqueue(getpid(), SIGRTMIN, v) < 0) die("sigqueue2");
      PHASE("f_sig_pending_l05"); phase_gap(gap_ms);
      if (sigqueue(getpid(), SIGRTMIN, v) < 0) die("sigqueue3");
      }
    PHASE("f_sig_pending"); phase_gap(gap_ms);
