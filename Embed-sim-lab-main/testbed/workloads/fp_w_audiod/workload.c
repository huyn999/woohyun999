 ...
 * workload: fp_w_audiod
 * scenario: [P] webOS-faithful: 오디오 데몬 — POSIX shm 버퍼(현대 스택 방식: SysV 아님) + 주기 믹서 + 버스
 * features: shm_posix,timerfd,thread1,unix_listen
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_shm_posix_l01"); phase_gap(gap_ms);
      shm_unlink(g_path_shm);
      PHASE("f_shm_posix_l02"); phase_gap(gap_ms);
      int fd = shm_open(g_path_shm, O_CREAT | O_RDWR, 0600); if (fd < 0) die("shm_open");
      PHASE("f_shm_posix_l03"); phase_gap(gap_ms);
      size_t n = 16ull << 20; if (ftruncate(fd, n) < 0) die("shm ftruncate");
      PHASE("f_shm_posix_l04"); phase_gap(gap_ms);
      char *p = mmap(NULL, n, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
      PHASE("f_shm_posix_l05"); phase_gap(gap_ms);
      if (p == MAP_FAILED) die("shm mmap");
      PHASE("f_shm_posix_l06"); phase_gap(gap_ms);
      for (size_t i = 0; i < n; i += 4096) p[i] = 3;
      PHASE("f_shm_posix_l07"); phase_gap(gap_ms);
      KEEP(p); KEEPFD(fd); }
    PHASE("f_shm_posix"); phase_gap(gap_ms);
    { int fd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK);
    PHASE("f_timerfd_l01"); phase_gap(gap_ms);
      if (fd < 0) die("timerfd_create");
      PHASE("f_timerfd_l02"); phase_gap(gap_ms);
      struct itimerspec its = { .it_interval = { 0, 100000000L }, .it_value = { 0, 100000000L } };
      PHASE("f_timerfd_l03"); phase_gap(gap_ms);
      if (timerfd_settime(fd, 0, &its, NULL) < 0) die("timerfd_settime");
      PHASE("f_timerfd_l04"); phase_gap(gap_ms);
      KEEPFD(fd); g_tfd = fd; }
    PHASE("f_timerfd"); phase_gap(gap_ms);
    ...
    PHASE("f_unix_listen_l01"); phase_gap(gap_ms);
      unlink(g_path_us);
      PHASE("f_unix_listen_l02"); phase_gap(gap_ms);
      int fd = socket(AF_UNIX, SOCK_STREAM, 0); if (fd < 0) die("us socket");
      PHASE("f_unix_listen_l03"); phase_gap(gap_ms);
      struct sockaddr_un a; memset(&a, 0, sizeof(a)); a.sun_family = AF_UNIX;
      PHASE("f_unix_listen_l04"); phase_gap(gap_ms);
      snprintf(a.sun_path, sizeof(a.sun_path), "%s", g_path_us);
      PHASE("f_unix_listen_l05"); phase_gap(gap_ms);
      if (bind(fd, (struct sockaddr *)&a, sizeof(a)) < 0) die("us bind");
      PHASE("f_unix_listen_l06"); phase_gap(gap_ms);
      if (listen(fd, 4) < 0) die("us listen");
      KEEPFD(fd); g_usfd = fd; }
    PHASE("f_unix_listen"); phase_gap(gap_ms);
