 ...
 * workload: fp_c_epg
 * scenario: EPG 캐시 서비스: 공유메모리 + 데이터 갱신 감시 + 캐시 파일 매핑
 * features: shm_posix,inotify,mmap_shared
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_inotify_l01"); phase_gap(gap_ms);
      mkdir(g_path_watch, 0755);
      PHASE("f_inotify_l02"); phase_gap(gap_ms);
      int fd = inotify_init1(IN_NONBLOCK); if (fd < 0) die("inotify_init1");
      PHASE("f_inotify_l03"); phase_gap(gap_ms);
      if (inotify_add_watch(fd, g_path_watch, IN_CREATE | IN_MODIFY) < 0) die("inotify_add_watch");
      PHASE("f_inotify_l04"); phase_gap(gap_ms);
      KEEPFD(fd); }
    PHASE("f_inotify"); phase_gap(gap_ms);
