 ...
 * workload: fp_x_s_o_tmpfile
 * scenario: v2 extension probe: O_TMPFILE 무명 파일 (이름이 애초에 없는 임시파일)
 * features: o_tmpfile
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_o_tmpfile_l01"); phase_gap(gap_ms);
      if (t >= 0) { if (write(t, "tmpdata", 7) != 7) die("o_tmpfile write");
      KEEPFD(t); }
      printf("PHASE_NOTE o_tmpfile ok=%d errno=%d\n", t >= 0, t >= 0 ? 0 : errno);
      PHASE("f_o_tmpfile_l02"); phase_gap(gap_ms);
      fflush(stdout); }
    PHASE("f_o_tmpfile"); phase_gap(gap_ms);
