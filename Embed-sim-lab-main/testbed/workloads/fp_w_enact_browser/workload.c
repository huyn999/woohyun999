 ...
 * workload: fp_w_enact_browser
 * scenario: [R] webOS-faithful: Enact 브라우저 — JIT + 힙 + 원격 연결: tcp l05 창의 webOS 재현(클러스터 ①) 포함
 * features: mmap_exec_jit,large_heap,memfd,tcp_established,unix_conn
...
int main(int argc, char **argv)
{
    ...
    { unsigned char *jit = mmap(NULL, 4096, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
      ...
      KEEP(jit); }
    PHASE("f_mmap_exec_jit"); phase_gap(gap_ms);
    { size_t n = 300ull << 20; char *p = malloc(n); if (!p) die("large_heap malloc");
    PHASE("f_large_heap_l01"); phase_gap(gap_ms);
      for (size_t i = 0; i < n; i += 4096) p[i] = 1;
      PHASE("f_large_heap_l02"); phase_gap(gap_ms);
      KEEP(p); }
    PHASE("f_large_heap"); phase_gap(gap_ms);
    { int fd = memfd_create("criuprobe", 0); if (fd < 0) die("memfd_create");
    PHASE("f_memfd_l01"); phase_gap(gap_ms);
      size_t n = 16ull << 20; if (ftruncate(fd, n) < 0) die("memfd ftruncate");
      PHASE("f_memfd_l02"); phase_gap(gap_ms);
      char *p = mmap(NULL, n, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
      PHASE("f_memfd_l03"); phase_gap(gap_ms);
      if (p == MAP_FAILED) die("memfd mmap");
      PHASE("f_memfd_l04"); phase_gap(gap_ms);
      for (size_t i = 0; i < n; i += 4096) p[i] = 2;
      PHASE("f_memfd_l05"); phase_gap(gap_ms);
      KEEP(p); KEEPFD(fd); }
    PHASE("f_memfd"); phase_gap(gap_ms);
    { int lp = 0; int l = probe_listen(0, &lp);
    PHASE("f_tcp_established_l01"); phase_gap(gap_ms);
      int cl = socket(AF_INET, SOCK_STREAM, 0); if (cl < 0) die("tcp cl socket");
      PHASE("f_tcp_established_l02"); phase_gap(gap_ms);
      struct sockaddr_in a; memset(&a, 0, sizeof(a)); a.sin_family = AF_INET;
      PHASE("f_tcp_established_l03"); phase_gap(gap_ms);
      a.sin_addr.s_addr = htonl(INADDR_LOOPBACK); a.sin_port = htons((uint16_t)lp);
      PHASE("f_tcp_established_l04"); phase_gap(gap_ms);
      if (connect(cl, (struct sockaddr *)&a, sizeof(a)) < 0) die("tcp connect");
      PHASE("f_tcp_established_l05"); phase_gap(gap_ms);
      int sv = accept(l, NULL, NULL); if (sv < 0) die("tcp accept");
      PHASE("f_tcp_established_l06"); phase_gap(gap_ms);
      if (write(cl, "hello", 5) != 5) die("tcp write");
      PHASE("f_tcp_established_l07"); phase_gap(gap_ms);
      KEEPFD(l); KEEPFD(cl); KEEPFD(sv); }
    PHASE("f_tcp_established"); phase_gap(gap_ms);
