 ...
 * workload: fp_s_sigaltstack
 * scenario: single-feature probe: 대체 시그널 스택 (크래시 핸들러 관례)
 * features: sigaltstack
...
int main(int argc, char **argv)
{
    ...
    PHASE("f_sigaltstack_l01"); phase_gap(gap_ms);
      if (!ss.ss_sp) die("sigaltstack malloc");
      PHASE("f_sigaltstack_l02"); phase_gap(gap_ms);
      if (sigaltstack(&ss, NULL) < 0) die("sigaltstack");
      KEEP(ss.ss_sp); }
    PHASE("f_sigaltstack"); phase_gap(gap_ms);
