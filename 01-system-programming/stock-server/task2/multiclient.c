...
int main(int argc, char **argv) 
{
	...
	char *host, *port, buf[MAXLINE], tmp[16];
	rio_t rio;
...
/*	fork for each client process	*/
	...
	/* 모든 client 종료 후 elapsed time + throughput 출력
	 throughput = (num_client * ORDER_PER_CLIENT) / elapsed_time
	 */
	gettimeofday(&end_time, NULL);
	elapsed_time = (end_time.tv_sec  - start_time.tv_sec) +
	               (end_time.tv_usec - start_time.tv_usec) / 1000000.0;

	long total_cmd = (long)num_client * ORDER_PER_CLIENT;
	double throughput = (elapsed_time > 0) ? (total_cmd / elapsed_time) : 0.0;

	printf("%.6f sec elapsed (clients=%d, order=%d), throughput=%.1f cmd/s\n",
	       elapsed_time, num_client, ORDER_PER_CLIENT, throughput);
